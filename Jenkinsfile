pipeline {
    ...
    environment {
        ...
        EC2_HOST = "13.234.20.73"      // Replace with your EC2 instance IP
    ...
    steps {
        script {
            ...
            sh '''
            scp -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkkins.pem -r app/main.py app/templates ubuntu@13.234.20.73:/home/ubuntu/USER-DETAILS/app/
            '''
            
            echo "Restarting application on EC2..."
            sh '''
            ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/jenkkins.pem ubuntu@13.234.20.73 <<EOF
